...
pub struct BridgeGuard {
    ...
    /// a closed channel and busy-spinning. Phase 4 drives foreground-task PTY
    /// forwarding through this sender.
...
impl TuiBridge {
    ...
    pub async fn connect(base_dir: &Path) -> Result<Self, ClientError> {
        ...
        // Event stream task: daemon /events → local rebroadcast. A
        // `ForegroundWaiting` event also triggers the terminal handoff — the
        // TUI releases its terminal, we bridge the task's PTY, then restore.
        tasks.push({
            let base = base.clone();
            let events_tx = events_tx.clone();
            let term_tx = terminal_request_tx.clone();
            tokio::spawn(async move {
                let client = Client::new(&base);
                let _ = client
                    .stream_events(|event| {
                        if let RunnerEvent::ForegroundWaiting { name } = &event {
                            let name = name.clone();
                            let base = base.clone();
                            let term_tx = term_tx.clone();
                            tokio::spawn(async move {
                                forward_foreground(&base, &name, &term_tx).await;
                            });
                        }
                        let _ = events_tx.send(event);
                    })
...
/// Hand the terminal from the TUI to a foreground task and back. Asks the TUI
/// to release its terminal (raw mode off, input task stopped), bridges the
/// task's PTY to the real terminal until it exits, then tells the TUI to
/// re-take the terminal.
async fn forward_foreground(base: &Path, name: &str, term_tx: &mpsc::Sender<TerminalRequest>) {
    let (ack_tx, ack_rx) = oneshot::channel();
    if term_tx.send(TerminalRequest::Acquire(ack_tx)).await.is_err() {
        return;
    }
    // Wait for the TUI to confirm it has released the terminal before we
    // touch raw mode / stdin.
    let _ = ack_rx.await;

    let socket = base.join(".don").join("don.sock");
    let _ = crate::client::foreground::run_foreground(&socket, name).await;

    // Give the terminal back to the TUI.
    let _ = term_tx.send(TerminalRequest::Release).await;
}
