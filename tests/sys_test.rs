...
macro_rules! process_test {
    ($name:ident, $body:block) => {
        #[test]
        fn $name() {
            run_with_timeout(Duration::from_secs(15), async $body);
        }
    };
}

process_test!(spawn_process_has_own_pgid, {
...
process_test!(spawn_pipe_mode_reads_output, {
...
process_test!(spawn_pty_mode_reads_output, {
...
process_test!(terminate_sends_sigterm_then_waits, {
    let args = ["300".to_string()];
    ...
    let status = handle
        .terminate(Signal::SIGTERM, Duration::from_secs(5))
        .await
        .unwrap();
    assert!(!status.success());
});

process_test!(terminate_escalates_to_sigkill_on_timeout, {
    ...
    let status = handle
        .terminate(Signal::SIGTERM, Duration::from_secs(1))
        .await
        .unwrap();
...
process_test!(pgid_file_written_on_spawn, {
...
process_test!(pgid_file_cleaned_up_on_drop, {
...
process_test!(pgid_file_read_and_cleanup, {
...
process_test!(pty_fallback_to_pipe_on_failure, {
...
process_test!(env_passed_to_child, {
